<template>
  <el-dialog
    ...
    <el-form-item label="陪诊员姓名" prop="name">
      <el-input v-model="dataForm.name" placeholder="陪诊员姓名"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          name: [
            { required: true, message: '陪诊员姓名不能为空', trigger: 'blur' }
          ],
          phone: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/fellow/fellowaddress/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.fellowId = data.fellowAddress.fellowId
                this.dataForm.name = data.fellowAddress.name
                this.dataForm.phone = data.fellowAddress.phone
                this.dataForm.postCode = data.fellowAddress.postCode
                this.dataForm.province = data.fellowAddress.province
                this.dataForm.city = data.fellowAddress.city
                this.dataForm.region = data.fellowAddress.region
                this.dataForm.detailAddress = data.fellowAddress.detailAddress
                this.dataForm.areacode = data.fellowAddress.areacode
                this.dataForm.defaultStatus = data.fellowAddress.defaultStatus
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/fellow/fellowaddress/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',

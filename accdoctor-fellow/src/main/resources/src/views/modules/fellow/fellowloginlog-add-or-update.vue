    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/fellow/fellowloginlog/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.fellowId = data.fellowLoginLog.fellowId
                this.dataForm.createTime = data.fellowLoginLog.createTime
                this.dataForm.ip = data.fellowLoginLog.ip
                this.dataForm.city = data.fellowLoginLog.city
                this.dataForm.loginType = data.fellowLoginLog.loginType
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/fellow/fellowloginlog/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',

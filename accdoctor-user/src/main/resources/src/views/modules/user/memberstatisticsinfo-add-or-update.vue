<template>
  <el-dialog
    ...
    <el-form-item label="关注的陪诊员数量" prop="collectFellowCount">
      <el-input v-model="dataForm.collectFellowCount" placeholder="关注的陪诊员数量"></el-input>
    </el-form-item>
    </el-form>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/user/memberstatisticsinfo/info/${this.dataForm.id}`),
              method: 'get',
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.collectFellowCount = data.memberStatisticsInfo.collectFellowCount
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/user/memberstatisticsinfo/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
              data: this.$http.adornData({
                ...
                'collectFellowCount': this.dataForm.collectFellowCount
              })
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.$message({

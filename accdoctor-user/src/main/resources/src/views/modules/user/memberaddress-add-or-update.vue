    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/user/memberaddress/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.memberId = data.memberAddress.memberId
                this.dataForm.name = data.memberAddress.name
                this.dataForm.phone = data.memberAddress.phone
                this.dataForm.postCode = data.memberAddress.postCode
                this.dataForm.province = data.memberAddress.province
                this.dataForm.city = data.memberAddress.city
                this.dataForm.region = data.memberAddress.region
                this.dataForm.detailAddress = data.memberAddress.detailAddress
                this.dataForm.areacode = data.memberAddress.areacode
                this.dataForm.defaultStatus = data.memberAddress.defaultStatus
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/user/memberaddress/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
